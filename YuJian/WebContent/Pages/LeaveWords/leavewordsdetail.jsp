...
<body>
	...
	<div class="wrapper leavewordPage">
		<div class="userleavewords-board">
			<!-- 留言 -->
			<div id="main-leaveword"></div>
			<div id="other-leaveword"></div>
		</div>
	</div>
	
	<div class="tabbar">
		<div style="width: 100%;">
			<button class="words-btn">回复</button>
		</div>
	...
	<script>
		var userid=${userid};
		var leavewordid='${leavewordid}';
		var myid=${myid};
		var page=-1;
		var toleaveworduserid=-1;
		var mianleaveword=null;
		
		$(function(){
			$.ajaxSetup({async : false });
			loadMainLeaveWord();	//加载被评论的留言
		...
		$('.wrapper').click(function(){
			...
			toleaveworduserid=mianleaveword.fromuserid;
			$('.words-btn').html('回复');
		})
		...
		function expressionClick(obj){
			...
			$.get('/YuJian/LeaveWords/AddLeaveWord?touserid='+userid
					...
					+'&parentid='+leavewordid
					,function(){						
						page=1;
						loadLeaveWords(function(){
							$('#other-leaveword').html('');
							$('.wrapper').click();
						});	//加载留言
			});
		}
		...
		function getLeaveWordsItem(item){
			var classname='';
			if(item.fromuserid==myid&&item.leavewordid!=leavewordid){
				classname='right-item';
			}
			var temp='<div class="leavewords-item '+classname+'"><div class="header">'
				...
				+'</div><a onclick="leavewordtoClick(this)" class="leavewordto" data-name="{2}" data-id="{0}">回复</a></div>'
		...
		function loadLeaveWords(callback){
			$.post('/YuJian/LeaveWords/GetUserLeaveWordsByParentId'
					,{parentid :leavewordid,page:page,pageSize:10}
					,function(data){
						closeSpin();
						if(callback!=null)
							callback();
						...
						$('#other-leaveword').append(html);
						//禁止传递事件
						$('.leavewords-item a').click(function(e){   
							e.stopPropagation();
						});
					},'json');
		}
		...
		//加载被评论的留言
		function loadMainLeaveWord(){
			$.post('/YuJian/LeaveWords/GetSingle'
					,{leavewordid :leavewordid}
					,function(data){
						mianleaveword=data;
						toleaveworduserid = data.fromuserid;
						
						$('#main-leaveword').append(getLeaveWordsItem(data));
						//禁止传递事件
						$('.leavewords-item a').click(function(e){   
							e.stopPropagation();
						});
