...
<body>
	...
	<div class="wrapper myinfo">
		...
		<div class="me-content">
			<a href="/YuJian/UpdateBaseInfo" class="header">基本资料 <i class="icon icon-arrow"></i></a>
		...
		<div class="me-content">
			<a href="/YuJian/UpdateSignContent" class="header">内心独白 <i class="icon icon-arrow"></i></a>
			<div id="signcontent">
				<!-- 内心独白 -->
				无
			</div>
		...
		<div class="me-content">
			<a href="/YuJian/UpdateSelectCondition" class="header">择偶条件 <i class="icon icon-arrow"></i></a>
			<div id="selectcondition">
				<label>无</label>
			</div>
		</div>
		<div class="me-content">
			<a href="/YuJian/UpdateHobby" class="header">兴趣爱好 <i class="icon icon-arrow"></i></a>
			<div id="hobby">
		...
		<div class="me-content">
			<a id="rank" style="display: none;" href="" class="header">礼物榜 <i class="icon icon-arrow"></i></a>
			<a id="matchmaker" style="display: none;" href="/YuJian/MyInfoMatchmaker" class="header">红娘信息设置 <i class="icon icon-arrow"></i></a>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function uploadVideo(){
         	...
         	//上传
         	$.ajaxFileUpload({
    			url: '/YuJianRoom/Common/UpdateVideo?userid='+user.userid+'&pwd='+user.password,
                type: 'post',
                secureuri: false,
		...
		function uploadImgs(file){
			...
			showSpin();
         	...
         	lrz(file,{quality:0.7}).then(function(data) {
                    //上传
                    $.ajax({
                        url: '/YuJianRoom/Common/UploadUserImgs?userid='+user.userid+'&pwd='+user.password,
                        async: true,
                        cache: false,
                        contentType: false,
                        processData: false,
                        ...
                        success: function (data){  //服务器成功响应处理函数
                        	...
                        	var temp='<div class="img-border"><img data-imgid="{0}" src="{1}" onclick="window.location.href=\'/YuJian/MyImgs\'" /></div>'
        					temp=temp.format(data.userimgid,url);
		...
		function loadMe(){
			$.post('/YuJian/Users/GetMyInformation'
				...
				$('#rank').attr('href','/YuJian/GiftRank?id='+user.userid)
				$('#rank').show();
			},'json');
		}
