...
<body>
	...
	<div class="wrapper hotPage">
		<div class="topbox" style="margin-top: -15px;">
			<div class="toptab">
				<a href="#" class="red">热度</a>
				<a href="/YuJian/SortUsers?isriches=1">财富</a>
			</div>
	...
	function loadData(){
		if(page<0)
			return;
		$.post("/YuJian/Users/GetUsersByHotCount"
				...
				,function(data){
					...
					for(var i=0;i<data.length;i++){
						var temp='<div class="rankline"><span>'
								...
								+'<a href="{4}">{1}</a>'
								+'<b><span class="icon-sort icon-flower"></span><i>{2}</i></b>'
								+'</label></span>'
		            	...
		            	if(data[i].usersortid==1){
		            		tempPrimary=tempPrimary.format(data[i].headimgurl==''?"/YuJian/Contents/images/dfthead.png":data[i].headimgurl
		            				,data[i].realname,data[i].hotcount,data[i].usersortid,'/YuJian/UserDetail?id='+data[i].userid);
		            		html+=tempPrimary;
		            	}
		            	else{
